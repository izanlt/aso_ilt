...
```powershell
$logok = foreach ($linea in $movimientosCrudos) {
    ...
    $qty = $partes[3].Split(':')[1]

    $fechaLimpia = $partes[1].Replace('_', ' ').Replace('-', '/')
    $TimeStamp = (Get-Date $fecha).ToString("yyyy-MM-dd HH:mm")

    $status = partes[4].Split(":")[1]
    if ($status -eq "Out_Of_Order") { $status = "Replace" }

     $actionRequired = switch ($status) {
        "Damaged"       { "Repair" }
...
$reporte | Export-Csv "reporte_inventario.csv" -NoTypeInformation
 ```