...
```bash
0 9 * 7-9 1-5 echo "$(date '+%Y-%m-%d %H:%M:%S')" >> /home/usuario/usuarios_conectados.log && who >> /home/usuario/usuarios_conectados.log
```
...
cron.d: Contiene ficheros con tareas programadas en formato crontab
cron.allow: Su función es contener el listado de usuarios autorixados para usar el cron
cron.deny: Contiene el listado de usuarios que tienen prohibido usar el cron
cron.daily: En él, hay scripts que se ejecutan una vez por día
cron.hourly: en el cron.hourly hay scripts que se ejcutan una vez por hora
cron.monthly: en el cron.monthly hay scripts que se ejecutan cada mes
