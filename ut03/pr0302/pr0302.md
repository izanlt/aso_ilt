...
case "$archivo" in
    *.txt)
        ...
        ;;
    *.zip)
        echo "Es un archivo ZIP (.zip)"
        ;;
    *.tar.gz)
        echo "Es un archivo TAR GZ (.tar.gz)"
        ;;
esac
...
case $opcion in
    ...
    2)
        ...
        ;;
    3)
        echo "Cerrando la sesión actual"
         pkill -KILL -u $USER
        ;;
    4)
...
#!/bin/bash
# Script que muestra el día de la semana según un número del 1 al 7

echo -n "Introduce un número del 1 al 7: "
read numero
...
```bash
if [ $? -eq 0 ]; then
    echo "La operación se ha realizado correctamente."
else
    echo "Error: la operación no se ha podido realizar."
fi
```
